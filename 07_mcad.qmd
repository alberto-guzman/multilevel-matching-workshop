...
1. `MatchPW()` (from `CMatching` [@arpino_propensity_2016], the package Chapter 5 used for site-restricted matching) requires its input vectors sorted in ascending order of `Group`, so we sort the teacher frame by `school_id`.
