...
3. Run `00_install_packages.R`. The last line should read `Setup complete`.
...
| Time | Activity |
|:------|:---------|
| 9:30 | Foundational concepts, the typology of designs, and the six stages (lecture), then Chapter 4, the data and the R packages |
| 10:20 | *Break. Finish installing if you have not.* |
| 10:30 | Chapter 5, the multisite individual assignment design, run together stage by stage |
| 11:12 | Chapter 6, the cluster assignment design, a tour with Stages 3 and 4 run together |
| 11:25 | Close and questions |

## How the session runs

Jordan opens with the lecture, which covers the foundational concepts, the typology of multilevel designs, and the six-stage matching workflow. He then shows the data and the packages from Chapter 4 of the handbook.
...
- **Chapter 6.** Partly a tour and partly hands-on. Alberto shows Stages 1 and 2 on the projector, you run Stages 3 and 4 with him, and he shows Stages 5 and 6. Run the whole page at home.
...
| [CAD](06_cad.qmd) | 6 | Whole schools are assigned to treatment. The cluster-level propensity score, cluster-global, sequential, and network-flow matching, balance at two levels, cluster-average and individual-average effects, permutation inference, and a sensitivity analysis. | After Chapter 5. Stages 1, 2, 5, and 6 on the projector; Stages 3 and 4 run together. Run the whole page at home. |
...
The [Slides](slides.qmd) page has Jordan's lecture slides for the first hour as a PDF and Alberto's short deck for the hands-on portion.
...
This is optional and for after the session. Everything we run in the room uses the TIMSS data in the repository. Your data should have one row per individual, with four kinds of columns.
